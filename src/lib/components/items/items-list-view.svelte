...
<div class="h-full space-y-2 grow">
	{#each items as item}
		<div
			...
			class={[
				'relative flex flex-col items-start py-1 px-2 space-y-2  rounded-sm bg-secondary bg-opacity-80 dark:bg-opacity-40 hover:bg-secondary/40 dark:hover:bg-secondary/60 group',
			...
			<div class="flex flex-wrap gap-2">
				{#each propertyState.properties as property (property.id)}
					{#if containsView(property.visibleInViews, View.LIST)}
						...
						{#if propertyRef}
							<PropertyValue itemId={item.id} {property} view={View.LIST} />
						{/if}
					{/if}
				{/each}
			</div>
